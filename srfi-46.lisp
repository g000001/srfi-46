...

(define-function (sid? sexp)
  (or (and (symbol? sexp)
           (not (null? sexp)))
      (renamed-sid? sexp)))
(define-function (renamed-sid? sexp)  (and (vector? sexp) (< 1 (vector-length sexp))))
(define-function (svector? sexp)      (and (vector? sexp) (= 1 (vector-length sexp))))
(define-function (svector->list sexp) (vector-ref sexp 0))
(define-function (list->svector l) (vector l))

(define-function (make-sid name renamed-id location)
  (if (eq? name location)
      (vector name renamed-id)
      (vector name renamed-id location) ))

(define-function (sid-name sid) (if (symbol? sid) sid (vector-ref sid 0)))
(define-function (sid-id sid)   (if (symbol? sid) sid (vector-ref sid 1)))
(define-function (sid-location sid)
  (if (symbol? sid) sid (vector-ref sid (if (= 2 (vector-length sid)) 0 2))) )

(define-function (list1? x) (and (pair? x) (null?  (cdr x))))
(define-function (list2? x) (and (pair? x) (list1? (cdr x))))
...
(define-function (map-vecs f x)
  ...
  (with-local-define-function
    ...
    (define-function (mv x) (or (mv2 x) x))
    :in
    (mv x) ) )

(define-function (wrap-vec v) (list->svector (wrap-vecs (vector->list v))))
(define-function (wrap-vecs input) (map-vecs #'wrap-vec input))
(define-function (unwrap-vec v-sexp)
  (if (= 1 (vector-length v-sexp))
      (list->vector (unwrap-vecs (svector->list v-sexp)))
      (vector-ref v-sexp 0) ))
(define-function (unwrap-vecs sexp) (map-vecs #'unwrap-vec sexp))

;; The store maps locations to vals.
;; vals are variables, syntaxes, or code.

(define-function (make-code output) (list output))
(define-function (make-builtin name) (list 'builtin name))
(define-function (make-transformer synrules env) (list synrules env))

(define-function (variable? val) (symbol? val))
(define-function (code? val) (list1? val))
(define-function (code-output code) (car code))

(define-function (syntax? val) (list2? val))

(define-function (builtin? syntax) (eq? 'builtin (car syntax)))
(define-function (builtin-name builtin) (cadr builtin))

(define-function (transformer? syntax) (not (builtin? syntax)))
(define-function (transformer-synrules trans) (car trans))
(define-function (transformer-env trans) (cadr trans))

(define-function (acons key val alist) (cons (cons key val) alist))

(cl:defconstant empty-env '())
(cl:defconstant empty-store '())
...
;;   2. For a local location, return 'NIL.  This can only happen for a
...
(define-function (lookup2 sid env store)
...
(define-function (extend-env env id location) (acons id location env))
(define-function (extend-store store loc val) (acons loc val store))
...
(define-function (substitute-in-store store loc val)
  ...
  (let ((str (symbol->string sym)))
    (with-local-define-function
      ...
      (cl:case sym
        ...
        (cl:otherwise (if (and (positive? (string-length str))
...
(define-function (make-begin outputs)
  (if (list1? outputs) (car outputs) (cons 'begin outputs)) )

(define-function (expand-lambda formals expr id-n env store loc-n)
  (with-local-define-function
    ;; (a b . c) => (a b c)
    (define-function (flatten-dotted x)
      (if (pair? x) (cons (car x) (flatten-dotted (cdr x))) (list x)) )
    ;; (a b c) => (a b . c)
    (define-function (dot-flattened x)
...
(define-function (check-syntax-bindings bindings)
  (or (list? bindings) (error "Non-list syntax bindings list: " bindings))
  (for-each (lambda (b) (or (and (list2? b)
                                 (sid? (car b)))
...
(define-function (expand-syntax-bindings bindings id-n syntax-env ienv store loc-n k)
  (let loop ((bs bindings) (vals '()) (store store) (loc-n loc-n))
    (if (not (null? bs))
	(expand-val (cl:cadar bs) id-n syntax-env store loc-n
...
;; encountered.  It also signals an error if ek, sk, dk, or bk is 'NIL
;; and the corresponding thing is encountered; however, if a begin is
;; encountered and bk is 'NIL, the begin is expanded as an expression
;; and passed to ek.
;;
;; lsd? == Let-Syntax around Definitions is okay.  If lsd? is 'NIL and a
;; let-syntax is encountered, it is assumed to start an expression or
;; syntax, so if ek and sk are 'NIL an error will be signalled.  lsd? is
;; only true at top-level.  (Let-syntax around internal definitions is
;; just too semantically bizarre.)
(define-function (expand-any sexp id-n env store loc-n lsd? ek sk dk bk)
  (with-local-define-function
    (define-function (get-k k sexp name)
      (or k (error (string-append name " used in bad context: ")
                   sexp )))
    (define-function (get-ek sexp) (get-k ek sexp "Expression"))
    (define-function (get-sk sexp) (get-k sk sexp "Syntax"))
    (define-function (get-dk sexp) (get-k dk sexp "Definition"))
    :in
    (let again ((sexp sexp) (id-n id-n) (store store) (loc-n loc-n))
         (with-local-define-function
           (define-function (expand-subexpr sexp) (expand-expr sexp id-n env store loc-n))
           (define-function (handle-syntax-use syntax head store loc-n)
             (let* ((tail (cdr sexp)) (sexp (cons head tail)))
               (if (transformer? syntax)
                   ...
                   (let ((builtin (builtin-name syntax)) (len (length tail)))
                     (with-local-define-function
                       (define-function (handle-macro-block)
                         (or ek sk lsd?
                       ...
                       (cl:case builtin
                         ...
                         ((syntax-rules)
                          ...
                          (let ((syn (compile-syntax-rules sexp env)))
                            (funcall (get-sk sexp) syn sexp store loc-n) ))
                         ((begin)
                          (cond (bk (funcall bk sexp id-n env store loc-n))
           ...
           (cond ((boolean? sexp)
                  ;; FIXME
                  (funcall (get-ek sexp) sexp))
                 ((sid? sexp)
                  (let ((val (lookup2 sexp env store)))
                    (if (syntax? val)
                        (funcall (get-sk sexp) val sexp store loc-n)
                        (funcall (get-ek sexp) (if (code? val) (code-output val) val)) )))
                 ((null? sexp) (error "Null used as an expression or syntax: " sexp))
                 ((list? sexp)
                  (expand-any (car sexp) id-n env store loc-n 'NIL
                              (and ek #'handle-combination) #'handle-syntax-use 'NIL 'NIL))
...
(define-function (expand-val sexp id-n env store loc-n k)
  (expand-any sexp id-n env store loc-n 'NIL
    (lambda (output) (funcall k (make-code output) store loc-n))
    (lambda (syn error-sexp store loc-n)
      (cl:declare (cl:ignore error-sexp))
      (funcall k syn store loc-n) )
    'NIL 'NIL))

(define-function (expand-expr sexp id-n env store loc-n)
  (expand-any sexp id-n env store loc-n 'NIL (lambda (x) x) 'NIL 'NIL 'NIL) )

;; args and return are as in expand-any.
(define-function (expand-body sexps id-n env store loc-n lsd? ek sk dk bk)
  (with-local-define-function
    ...
    (define-function (expand-def sexp vds sds exprs id-n env store loc-n k ek)
      (with-local-define-function
        (define-function (dk builtin sexp id-n env store loc-n)
          (if (list2? sexp) ;; A (define <expression>) form.
              (if exprs
                  (funcall k vds sds (cons (cadr sexp) exprs) id-n env store loc-n)
                  (error "Non-syntax definition in a syntax body: " sexp) )
              (let* ((sid (cadr sexp))
                     (id (sid-id sid))
                     (env (extend-env env id loc-n)) )
                (with-local-define-function
                  (define-function (check def)
                  ...
                  (begin
                    ...
                    (cl:case builtin
                      ((define-syntax)
                       (funcall k vds (cons sexp sds) exprs id-n env store (+ loc-n 1)) )
                      ((define)
                       (or exprs
                       ...
                       (let* ((var (intloc->var loc-n sid))
                              (store (extend-store store loc-n var))
                              (loc-n (+ loc-n 1)) )
                         (funcall k (cons sexp vds) sds exprs id-n env store loc-n) ))))))))
        (define-function (bk sexp id-n env store loc-n)
          (let loop ((sexps (cdr sexp)) (vds vds) (sds sds) (exprs exprs)
                     (id-n id-n) (env env) (store store) (loc-n loc-n) (ek ek))
               (if (null? sexps)
                   (funcall k vds sds exprs id-n env store loc-n)
                   (expand-def (car sexps) vds sds exprs id-n env store loc-n
                               (lambda (vds sds exprs id-n env store loc-n)
                                 (loop (cdr sexps) vds sds exprs id-n env store loc-n 'NIL) )
                               (and ek (lambda (out)
                                         (with-local-define-function
                                           (define-function (expand-one sexp)
                                             (expand-expr sexp id-n env store loc-n) )
                                           :in
                                           (let ((rest (map #'expand-one (cdr sexps))))
                                             (funcall ek (make-begin (cons out rest))) ))))))))
        :in
        (expand-any sexp id-n env store loc-n 'NIL ek 'NIL #'dk #'bk) ))
    :in
    (let loop ((first (car sexps)) (rest (cdr sexps))
               (vds '()) (sds '()) (exprs (and ek '()))
               (id-n id-n) (env env) (store store) (loc-n loc-n))
         (with-local-define-function
           (define-function (finish-body boundary-exp-output)
             (expand-syntax-bindings (map #'cdr sds) id-n env env store loc-n
                                     (lambda (store loc-n)
                                       (with-local-define-function
                                         (define-function (iexpand sexp) (expand-expr sexp id-n env store loc-n))
                                         (define-function (expand-vd vd)
                                           (list (lookup2 (cadr vd) env store) (iexpand (cl:caddr vd))) )
                                         (define-function (make-letrec bindings expr)
...
(define-function (expand-top-level-forms forms store loc-n k)
  (with-local-define-function
    (define-function (finalize store loc-n acc)
      (funcall k (reverse acc) store loc-n) )
    :in
    ...
    (let expand ((sexps (wrap-vecs forms)) (id-n 0) (env empty-env)
         ...
         (if (null? sexps)
             (funcall k store loc-n acc)
             (let ((rest (cdr sexps)))
               (with-local-define-function
                 (define-function (ek output)
                 ...
                 (define-function (bk sexp id-n* env* store loc-n)
                   (expand (cdr sexp)  id-n* env* store loc-n acc
                           (lambda (store loc-n acc)
                             (expand rest id-n env store loc-n acc k) )))
                 :in
                 (expand-any (car sexps) id-n env store loc-n 'T #'ek 'NIL #'dk #'bk) ))))))
...
(define-function (compile-syntax-rules synrules env)
  (let ((ellipsis-id (and (pair? (cl:cddr synrules))
                          (sid? (cadr synrules))
                          (sid-id (cadr synrules)) )))
    (with-local-define-function
      (define-function (ellipsis? x)
        (and (sid? x)
             (if ellipsis-id
                 (eqv? ellipsis-id (sid-id x))
                 (eq? '*** (lookup-sid x env)) )))

      (define-function (check-lit lit)
        (or (sid? lit)
            (error "Non-id: " lit " in literals list of: " synrules) )
        (if (ellipsis? lit)
            (error "Ellipsis " lit " in literals list of: " synrules) ))
      :in
      (let* ((rest (if ellipsis-id (cl:cddr synrules) (cdr synrules)))
        ...
        (with-local-define-function
          (define-function (ellipsis-pair? x)
            (and (pair? x) (ellipsis? (car x))) )

          (define-function (check-ellipses pat/tmpl in-template?)
            (with-local-define-function
              (define-function (bad-ellipsis x reason)
                (error (string-append reason ": ")
                       x
                       (if in-template? " in template: " " in pattern: ")
                       pat/tmpl ))

              (define-function (multi-ellipsis-error x)
                (bad-ellipsis x "List or vector pattern with multiple ellipses") )

              (define-function (ellipsis/tail-error x)
                (bad-ellipsis x "Improper list pattern with an ellipsis") )

              (define-function (ellipsis-follows x thing)
                ...
                (let check ((x x))
                     (cond ((pair? x)
                            ...
                            (if (ellipsis-pair? (cdr x))
                                (cond ((ellipsis? (cl:cddr x))
                                       (ellipsis-follows (cdr x) "a '.'") )
                                      ((ellipsis-pair? (cl:cddr x))
                                       (ellipsis-follows (cdr x) "an ellipsis") )
                                      (in-template? (check (cl:cddr x)))
          ...
          (define-function (make-pat-env pat)
            (let collect ((x (cdr pat)) (depth 0) (l '()))
                 (cond ((sid? x)
                       ...
                       ((pair? x)
                        (if (ellipsis-pair? (cdr x))
                            (collect (car x) (+ 1 depth) (collect (cl:cddr x) depth l))
                            (collect (car x) depth (collect (cdr x) depth l)) ))
                       (else l) )))

          ;; Checks var depths.
          (define-function (check-var-depths tmpl pat-env)
            (with-local-define-function
              (define-function (depth-error x)
                (error "Pattern var used at bad depth: " x " in template: " tmpl) )
              (define-function (close-error x)
                   ...
                   (cond ((sid? x)
                         ...
                         ((pair? x)
                          (let* ((ellip? (ellipsis-pair? (cdr x)))
                                 ...
                                 (cdr-closed?
                                  (collect (funcall (if ellip? #'cl:cddr #'cdr) x)
                                           depth )))
          ...
          (define-function (check-rule rule)
          ...
          (define-function (reduce-env lits)
            (with-local-define-function
              (define-function (list-dots-ids x ids)
                       ...
                       (loop (cdr ids)
                             (let ((id (car ids)))
                               (cond ((and (not (assv id reduced-env)) (assv id env))
                                      => (cl:lambda (binding) (cons binding reduced-env))) ;cl:lambda needed
                                     (else reduced-env) )))))))
          :in
...
;; returns (k sexp id-n)
(define-function (apply-transformer transformer sexp id-n env k)
  (let* ((synrules (transformer-synrules transformer))
	 (mac-env (transformer-env transformer))
	 (ellipsis-id (and (sid? (cadr synrules))
			   (sid-id (cadr synrules)) ))
	 (rest (if ellipsis-id (cl:cddr synrules) (cdr synrules)))
	 (pat-literals (map #'sid-id (car rest)))
	 (rules (cdr rest)) )

    (with-local-define-function
      (define-function (pat-literal? id)     (memv id pat-literals))
      (define-function (not-pat-literal? id) (not (pat-literal? id)))
      (define-function (ellipsis-pair? x)    (and (pair? x) (ellipsis? (car x))))
      (define-function (ellipsis? x)
        (and (sid? x)
             (if ellipsis-id
                 (eqv? ellipsis-id (sid-id x))
      ...
      (define-function (list-ids x include-scalars pred?)
        (let collect ((x x) (inc include-scalars) (l '()))
             (cond ((sid? x) (let ((id (sid-id x)))
                               (if (and inc (funcall pred? id)) (cons id l) l) ))
                   ((vector? x) (collect (svector->list x) inc l))
                   ((pair? x)
                    (if (ellipsis-pair? (cdr x))
                        (collect (car x) 'T (collect (cl:cddr x) inc l))
                        (collect (car x) inc (collect (cdr x) inc l)) ))
                   (else l) )))

      (define-function (matches? pat)
      ...
      (define-function (make-bindings pat)
        (let collect ((pat pat) (sexp (cdr sexp)) (bindings '()))
             (cond ((and (sid? pat) (not (pat-literal? (sid-id pat))))
                   ...
                   ((ellipsis-pair? (cdr pat))
                    (let* ((tail-len (length (cl:cddr pat)))
      ...
      (define-function (expand-template pat tmpl top-bindings)
        (let ((tmpl-literals
               (remove-dups (list-ids tmpl 'T
                                      (lambda (id) (not (assv id top-bindings))) )))
              (ellipsis-vars (list-ids pat 'NIL #'not-pat-literal?)) )
          (with-local-define-function
            (define-function (list-ellipsis-vars subtmpl)
              (list-ids subtmpl 'T (lambda (id) (memv id ellipsis-vars))) )
            (define-function (expand tmpl bindings)
              (let expand-part ((tmpl tmpl))
                   (cond
                     ...
                     ((pair? tmpl)
                      (if (ellipsis-pair? (cdr tmpl))
                          (let ((vars-to-iterate (list-ellipsis-vars (car tmpl))))
                            (with-local-define-function
                              (define-function (lookup var) (cdr (assv var bindings)))
                              (define-function (expand-using-vals . vals)
                     ...
                     (else tmpl) )))
            :in
            (funcall k (expand tmpl top-bindings) (+ id-n (length tmpl-literals))) )))
      :in
      (let loop ((rules rules))
           (if (null? rules)
               (error "No matching rule for macro use: " sexp)
               (let* ((rule (car rules)) (pat (cl:cdar rule)) (tmpl (cadr rule)))
                 (if (matches? pat)
	...
	(define-protected-macros letrec-syntax
	  ...
	  (define-syntax and
	    (syntax-rules ()
	      ((_) 'T)
	      ((_ test) (let () test))
	      ((_ test . tests) (if test (and . tests) 'NIL))))
	  (define-syntax or
	    (syntax-rules ()
	      ((_) 'NIL)
	      ((_ test) (let () test))
...
(cl:defparameter null-stuff
  (expand-top-level-forms null-prog builtins-store 0 #'list))

(cl:defparameter null-output (car null-stuff))
(cl:defparameter null-store  (cadr null-stuff))
(cl:defparameter null-loc-n  (cl:caddr null-stuff))

(define-function (expand-program forms)
  (expand-top-level-forms forms null-store null-loc-n
    (lambda (outputs store loc-n)
      (cl:declare (cl:ignore store loc-n))
      (append null-output outputs))))

;; an mstore is a mutable store.
(define-function (null-mstore) (cons null-store null-loc-n))

(define-function (expand-top-level-forms! forms mstore)
...
(cl:defparameter repl-mstore (null-mstore))
...
'
(begin
  (define-function (file->list file)
    (define-function (f) (let ((x (read))) (if (eof-object? x) '() (cons x (f)))))
    (with-input-from-file file f))

  (define-function (check-expander filename)
...
'(let ((multiplier 2))
   (define-function ((curried-* x) y) (* x y))
...
;; 		        (lambda (tmp ***) (lambda () 'NIL (set! var tmp) ***))))
;;      ***
;;      (thunk) *** (let () . body))))
...
'(define-syntax safe-letrec
  (let-syntax ()
	...
	((safe-letrec-with-alt-names (var* ***) ((var init) ***) . body)
	 (let ((var 'NIL) *** (ready? 'NIL))
	   (define-function (set-em! var* ***)
	     (set! var var*) *** (set! ready? 'T))
	   (define-function (var* new-val)
...
'(define-syntax safe-letrec*
  (let-syntax ()
    (define-syntax process-bindings
      (syntax-rules ()
	((process-bindings) 'NIL)
	((process-bindings binding *** (var init))
	 (let ((ready? 'NIL))
	   (define-function (finish val) (set! var val) (set! ready? 'T))
	   (define-function (safe-setter new-val)
	     (if ready? (set! var new-val) (letrec-set!-error 'var new-val)))
	   (let-syntax ((var (if ready? var (letrec-access-error 'var))))
	     (define-syntax new-set!
	       (let-syntax ((set! set!))
